...
Check the computation of Qx using just chromosome 3 
...
# Read in betas
betas <- fread("../output/transfer/ukb_imp_chr3_v3.Height-Long.betas.gz")

# Flip betas to get the effect size of the ALT allele
betas <- betas %>% mutate(BETA_Strat = case_when(ALT == A1 ~ BETA, REF == A1 ~ -1 * BETA))

# Read in Genotypes
X <- read_genos("../output/transfer/hgdp_wgs.20190516.full.chr3", betas_id = betas$ID)
...
TV <- fread("../output/transfer/Tvec_cordinates.txt.gz")
