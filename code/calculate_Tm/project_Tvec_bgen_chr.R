...
compute_b <- function(path_to_test, path_to_gwas, path_to_testvec, test_type, outpath) {
  ...
  cmd_b <- paste("sh code/calculate_Tm/GWAS_bgen_score.sh", path_to_gwas, paste0(outpath, "xt_temp.", test_type, ".glm.linear"), outfile_b, overlap_snps, sample_file, pheno_ID_file, sep = " ")
  system(cmd_b)
